...
def clean_text_COS(filename, acc=True, pun=True, low=True):
    ...
    for balise in tree.xpath('//rubrique[@lan="co"]/texte'):
        ...
        y = re.sub(r" vosc (\w*u\b)", r" vosciu \1", y)
        y = re.sub(r" vosc (\w*a\b)", r" voscia \1", y)
        y = re.sub(r" vosc (\w*i\b)", r" vosci \1", y)
        y = re.sub(r" vosc (\w*e\b)", r" vosce \1", y)
        y = re.sub(r" vosc (\w*à\b)", r" voscia \1", y)
        y = re.sub(r" vosc (\w*j\b)", r" vosci \1", y)

        y = re.sub(r" nosc (\w*u\b)", r" nosciu \1", y)
        y = re.sub(r" nosc (\w*a\b)", r" noscia \1", y)
        y = re.sub(r" nosc (\w*i\b)", r" nosci \1", y)
        y = re.sub(r" nosc (\w*e\b)", r" nosce \1", y)
        y = re.sub(r" nosc (\w*à\b)", r" noscia \1", y)
        y = re.sub(r" nosc (\w*j\b)", r" nosci \1", y)
