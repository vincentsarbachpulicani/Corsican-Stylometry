...
class TopicModeling_Gensim:
    """
    Initiate the topic modeling process using the Gensim library and the LDA module. The only parameter need is a corpus of tokenized documents. The differents methods existing are here to refine the results of the TM.
    ...
    def vocabulary_from_corpus(self, filter_ext = True, no_below=5, no_above=0.5):
        """
    ...
    def lda_model_training(self, num_topics, chunksize = 1500, passes = 20, iterations = 400, eval_every = None):
        """
        Method to train a LDA model from our corpus and data defined previously. 
        ...
        print("The LDA model has been trained successfully.")
        
    def lsa_model_training(self, num_topics, num_words):
        """
        Method to train a LSA model from our corpus and data defined previously. 
        ...
        model = LsiModel(co, num_topics=num_topics, id2word = self.dictionary)  # train model
        print(f"Topics of the model ---> {model.print_topics(num_topics=num_topics, num_words=num_words)}")
        
        
        self.model = model
        self.co = co
        self.model_type = str(type(model))
        print("\n\nThe LSA model has been trained successfully.")

    def top_words_topics(self, top_words, output="dataframe"):
        """
        ...
        top_words -> int : most frequent words
        output -> str : choose output, in "dataframe" or as an "image"
        """
        ...
        for num in range(self.model.num_topics):
            if self.model_type == "<class 'gensim.models.lsimodel.LsiModel'>":
                topic = self.model.show_topic(topicno=num, topn=top_words)

            elif self.model_type == "<class 'gensim.models.ldamodel.LdaModel'>":
                topic = self.model.show_topic(topicid=num, topn=top_words)
            ...
            dic[f"Topic {num + 1}"] = word_list

        for w in range(top_words):
            index.append(f"Word {w + 1}")

        df_coherence = pd.DataFrame(data=dic, index=index)
        ...
        if output == "dataframe":
            ...
            ax.table(cellText = df_coherence.values,
                      rowLabels = df_coherence.index,
                      colLabels = df_coherence.columns,
                      loc = "center"
                     )
    ...
    def draw_word_clouds(self, y, x, figsize=(20,10), dpi=150):
        ...
        for i in range(self.model.num_topics):
            ...
            dic = dict(self.model.show_topic(i, 30))
            for key in dic.keys():
                dic[key] = abs(dic[key])

            wordcloud = WordCloud(width=600, height=600, background_color='white').fit_words(dic)
